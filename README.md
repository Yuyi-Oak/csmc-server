...
- In-match actionbar HUD now shows compact phase code (`FRZ/BUY/LIVE/PLANT`) and concise side code (`T/CT/SPEC`).
...
- Round phase/actionbar notifications are now session-scoped to avoid cross-match message bleed in concurrent games.
...
- Grenade projectiles now carry session metadata so delayed hits stay confined to the original match even if thrower disconnects.
- Ongoing smoke/molotov area effects now self-cancel when the owning round leaves live combat state.
...
- `/csmc ac status [player]` now includes per-player top trigger reasons summary.
